...
class FridaLoader:
    def __init__(self, package_name, pid, jsFilePath):
        ...
        self.jsFile_getSign_model = None  # 用于初始化时存放获取sign值的js文件内容
        self.jsFile_doMatchV2ByDataDecryptCommand_model = None  # 用于初始化时存放解密试题的js文件内容
        self.jsFile_doAnswerEncrypt = None  # 用于初始化时存放加密答案的js文件内容
        update_js_file(self)  # 用于初始化时读取并预存js脚本内容
    ...
    def get_sign(self, url):
        ...
        # 从模板加载生成sign的js文件内容
        getSign_script_content = self.jsFile_getSign_model
        getSign_script_content_replace = getSign_script_content.replace("flag_url", target_url)
        self.script = self.session.create_script(getSign_script_content_replace)
    ...
    def get_question(self, reponse_base64):
        ...
        # 从模板加载解密试题的js文件内容
        dataDecrypt_script_content = self.jsFile_doMatchV2ByDataDecryptCommand_model
        dataDecrypt_script_content_replace = dataDecrypt_script_content.replace("flag_base64", reponse_base64)
        self.script = self.session.create_script(dataDecrypt_script_content_replace)
    ...
    def get_request_encrypt(self, answer_base64):
        ...
        # 加载js文件
        dataEncrypt_script_content = self.jsFile_doAnswerEncrypt
        dataEncrypt_script_content_replace = dataEncrypt_script_content.replace("flag_answer_base64", answer_base64)
        self.script = self.session.create_script(dataEncrypt_script_content_replace)
